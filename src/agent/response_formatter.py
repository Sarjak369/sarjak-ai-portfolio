...
class ResponseFormatter:
    ...
    def format_projects(self) -> str:
        ...
        <div style="padding: 20px;">
            <h2 style="color: #ececf1; margin-bottom: 24px; font-size: 24px;">🚀 Featured Projects</h2>
        """

        if self.projects and "projects" in self.projects:
            for i, project in enumerate(self.projects["projects"], 1):
                tech_tags = "".join([
                    f'<span style="display: inline-block; background: #40414f; color: #c5c5d2; padding: 4px 10px; border-radius: 12px; font-size: 12px; margin: 4px 4px 4px 0; border: 1px solid #565869;">{tech}</span>'
                    for tech in project.get("technologies", [])
                ])

                html += f"""
                <div style="background: #2f2f2f; padding: 24px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #3e3e3e;">
                    <h3 style="color: #ececf1; margin-bottom: 8px; font-size: 18px;">{i}. {project.get('name', '')}</h3>
                    <p style="color: #8e8ea0; font-size: 14px; margin-bottom: 12px; font-style: italic;">{project.get('tagline', '')}</p>
                    <p style="color: #c5c5d2; line-height: 1.6; margin-bottom: 12px;">{project.get('description', '')}</p>
    ...
    def format_experience(self) -> str:
        ...
        html = """
        <div style="padding: 20px;">
            <h2 style="color: #ececf1; margin-bottom: 24px; font-size: 24px;">💼 Professional Experience</h2>
        """

        if self.experience and "positions" in self.experience:
            for position in self.experience["positions"]:
                highlights_html = "".join([
                    f'<li style="margin-bottom: 8px; color: #c5c5d2;">{highlight}</li>'
                ...
                html += f"""
                <div style="background: #2f2f2f; padding: 24px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #3e3e3e;">
                    <h3 style="color: #ececf1; margin-bottom: 4px; font-size: 18px;">{position.get('title', '')} @ {position.get('company', '')}</h3>
                    <p style="color: #8e8ea0; font-size: 14px; margin-bottom: 12px;">{position.get('duration', '')} | {position.get('location', '')}</p>
    ...
    def format_skills(self) -> str:
        ...
        html = """
        <div style="padding: 20px;">
            <h2 style="color: #ececf1; margin-bottom: 24px; font-size: 24px;">🎯 Technical Skills</h2>
        """

        if self.skills and "categories" in self.skills:
            for category in self.skills["categories"]:
                skill_tags = "".join([
                    f'<span style="display: inline-block; background: #40414f; color: #c5c5d2; padding: 6px 12px; border-radius: 12px; font-size: 13px; margin: 4px 4px 4px 0; border: 1px solid #565869;">{skill}</span>'
                    for skill in category.get("skills", [])
                ])

                html += f"""
                <div style="background: #2f2f2f; padding: 24px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #3e3e3e;">
                    <h3 style="color: #ececf1; margin-bottom: 16px; font-size: 18px;">{category.get('name', '')}</h3>
                    <div>
                        {skill_tags}
                    </div>
                </div>
    ...
    def format_education(self) -> str:
        ...
        html = """
        <div style="padding: 20px;">
            <h2 style="color: #ececf1; margin-bottom: 24px; font-size: 24px;">🎓 Education</h2>
        """
        ...
        if self.education and "degrees" in self.education:
            for degree in self.education["degrees"]:
                ...
                html += f"""
                <div style="background: #2f2f2f; padding: 24px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #3e3e3e;">
                    <h3 style="color: #ececf1; margin-bottom: 4px; font-size: 18px;">{degree.get('degree', '')}</h3>
                    <p style="color: #8e8ea0; font-size: 14px; margin-bottom: 8px;">{degree.get('institution', '')} | {degree.get('location', '')}</p>
                    <p style="color: #c5c5d2; margin-bottom: 4px;">📅 {degree.get('duration', '')}</p>
                    <p style="color: #c5c5d2; margin-bottom: 12px;">📊 GPA: {degree.get('gpa', '')}</p>
                    <p style="color: #8e8ea0; font-size: 13px;"><strong>Relevant Courses:</strong> {courses}</p>
                </div>
                """
    ...
    def format_contact(self) -> str:
        ...
        html = f"""
        <div style="padding: 20px;">
            <h2 style="color: #ececf1; margin-bottom: 24px; font-size: 24px;">📧 Contact Information</h2>
            <div style="background: #2f2f2f; padding: 24px; border-radius: 12px; border: 1px solid #3e3e3e;">
                <p style="color: #c5c5d2; margin-bottom: 12px; font-size: 15px;">
